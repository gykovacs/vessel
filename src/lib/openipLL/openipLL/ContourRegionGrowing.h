...
namespace openip
{
	...
	if ( fTerm )
	{
	  ...
	  for ( int i= 0; i < f.size(); ++i )
	    ...
	    printf("number of filters: %zd %zd\n", filters.size(), pfilters.size());
	}
    ...
    int ContourRegionGrowingTowardsEqualSlope<INPUT>::isOuterContour4(Image<unsigned char>& image, int p)
    {
        ...
        for ( unsigned int i= 0; i < pos4.size(); ++i )
            if ( p + pos4(i) >= 0 && p + pos4(i) < int(image.n) && image(p + pos4(i)) == 255 && image(p) != 255 )
                return 1;
        return 0;
    }
    ...
    int ContourRegionGrowingTowardsEqualSlope<INPUT>::isInnerContour4(Image<unsigned char>& image, int p)
    {
        ...
        for ( unsigned int i= 0; i < pos4.size(); ++i )
            if ( p + pos4(i) >= 0 && p + pos4(i) < int(image.n) && !image(p + pos4(i)) && image(p) )
                return 1;
        return 0;
    }
	...
	for ( int i= 0; i < int(labels.n); ++i )
	{
	  int num= 0;
	    ...
	    filterResponses(i)= 0;
	    maxResponses(i)= 0;
	    for ( unsigned int j= 0; j < filters.size() && !flag; ++j )
	    {
	      if ( i + filters(j)->getMin() >= 0 && i + filters(j)->getMax() < int(input.n) )
	      {
...
//		printf("%f ", actCorr); fflush(stdout);
	    ...
	    for ( unsigned int j= 0; j < pfilters.size() && !flag; ++j )
	    {
	      if ( i + pfilters(j)->getMin() >= 0 && i + pfilters(j)->getMax() < int(input.n) )
	      {
...
//	    printf("%f ", filterResponses(i)); fflush(stdout);
    ...
    void ContourRegionGrowingTowardsEqualSlope<INPUT>::analyze(Image<INPUT>& input, Image<unsigned char>& labels)
    {
      ...
      for ( unsigned int i= 20000; i < labels.n - 20000; ++i )
      {
	if ( isInnerContour4(labels, i) )
	{
	...
	for ( unsigned int i= 0; i < contour.size(); ++i )
	{
	  if ( !roi || (*roi)(contour(i)) )
	  {
	    ...
	    for ( unsigned int j= 0; j < ends.size(); ++j )
	    {
	    ...
	    for ( unsigned int j= 0; j < filters.size() && !flag; ++j )
	    {
	      if ( contour(i) + filters(j)->getMin() >= 0 && contour(i) + filters(j)->getMax() < int(input.n) )
	      {
	    ...
	    for ( unsigned int j= 0; j < pfilters.size() && !flag; ++j )
	    {
	      if ( contour(i) + pfilters(j)->getMin() >= 0 && contour(i) + pfilters(j)->getMax() < int(input.n) )
	      {
    ...
    void ContourRegionGrowingTowardsEqualSlope<INPUT>::circularityConstraint(Image<unsigned char>& output, PixelSet1& final)
    {
        ...
        for ( unsigned int i= 0; i < final.size(); ++i )
        {
            ...
            float perimeter= 0;
            for ( unsigned int j= 0; j < sed.size(); ++j )
                if ( final(i) + sed(j) >= 0 && final(i) + sed(j) < int(output.n) && output(final(i) + sed(j)) > 0 )
                {
                    ++area;
    ...
    Vector<Region2> ContourRegionGrowingTowardsEqualSlope<INPUT>::extractOrderedContours(Region2 object, Image<unsigned char>& mask)
    {
        ...
        while ( 1 )
        {
            ...
            for ( unsigned int i= 0; i < tmpImage.n; ++i )
            ...
            while ( 1 )
            {
                ...
                for ( j= 0; j < pos.size(); ++j )
                {
                    ...
                    if ( pix >= 0 && pix < int(tmpImage.n) && isOuterContour8(tmpImage, pix) )
                    {
                        pm1= p;
                        p= pix;
  ...
  return 0;
	...
	if ( lTerm ) ++nTerms;
	    ...
	    if ( ! aTerm && ! mTerm && ! iTerm)
	    {
		  ...
		  printf("noc: %zd,%zd ", contours.size(), contours(0).size());
	  ...
	  for ( float threshold= 0.4; threshold <= 1; threshold+= 0.01 )
	  {
	    ...
	    for ( unsigned int i= 0; i < weights.n; ++i )
	    ...
	    //for ( float threshold2= threshold*0.8; threshold2 <= threshold; threshold2+= 0.05 )
	    {
	      ...
	      printf("%zd %zd ", regions.size(), segments.size());
