 ...
 * Author: gykovacs
 *
 * Created on July 20, 2011, 3:56 PM
 */
...
namespace openip
{
    ...
    class Transform3: public VectorTransform3<INPUT, OUTPUT>
    {
    public:
        using VectorTransform3<INPUT, OUTPUT>::descriptor;
        using VectorTransform3<INPUT, OUTPUT>::apply;
        using VectorTransform3<INPUT, OUTPUT>::getProposedBorder;
        ...
        virtual void apply(Volume<INPUT>& input, VolumeVector<OUTPUT>& output, int n, Volume<unsigned char>* roi= NULL, Volume<unsigned char>* support= NULL);

#ifdef USE_OPENCL        
        virtual std::string getOpenCLKernel();
        
        virtual int applyGPU(Volume<INPUT>& input, Volume<OUTPUT>& output, Volume<unsigned char>* roi= NULL, Volume<unsigned char>* support= NULL);
        
        virtual void enqueueKernel(cl_context* cc, cl_command_queue* cq, cl_kernel* k, void* bufin, void* bufout, void* bufroi, Volume<INPUT>& input);
#endif
        /**
     ...
     */
    template<typename INPUT, typename OUTPUT>
    std::ostream& operator<<(std::ostream& out, Transform3<INPUT, OUTPUT>& t3)
    {
        out << t3.descriptor;
        return out;
    }
    ...
    template<typename INPUT, typename OUTPUT>
    Transform3<INPUT, OUTPUT>::Transform3()
    : VectorTransform3<INPUT, OUTPUT>()
    {
    }

    template<typename INPUT, typename OUTPUT>
    Transform3<INPUT, OUTPUT>::Transform3(const Transform3& t)
    ...
    template<typename INPUT, typename OUTPUT>
    void Transform3<INPUT, OUTPUT>::apply(Volume<INPUT>&, Volume<OUTPUT>&, Volume<unsigned char>*, Volume<unsigned char>*)
    {
    }

    template<typename INPUT, typename OUTPUT>
    void Transform3<INPUT, OUTPUT>::apply(Volume<INPUT>& input, VolumeVector<OUTPUT>& output, int n, Volume<unsigned char>* roi, Volume<unsigned char>* support)
    {
    ...
    #ifdef USE_OPENCL
    template<typename INPUT, typename OUTPUT>
    std::string Transform3<INPUT, OUTPUT>::getOpenCLKernel()
    {
        return std::string("");
    }
    #endif
    ...
    #ifdef USE_OPENCL
    template<typename INPUT, typename OUTPUT>
    int Transform3<INPUT, OUTPUT>::applyGPU(Volume<INPUT>& input, Volume<OUTPUT>& output, Volume<unsigned char>* roi, Volume<unsigned char>*)
    {
    ...
    #ifdef USE_OPENCL
    template<typename INPUT, typename OUTPUT>
    void Transform3<INPUT, OUTPUT>::enqueueKernel(cl_context* , cl_command_queue* cq, cl_kernel* k, void* bufin, void* bufout, void* , Volume<INPUT>& input)
    {
