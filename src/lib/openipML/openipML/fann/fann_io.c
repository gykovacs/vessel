...
struct fann *fann_create_from_fd(FILE * conf, const char *configuration_file)
{
	...
	if ( !fscanf(conf, "cascade_activation_functions=") ) printf("fread error\n");
	for(i = 0; i < ann->cascade_activation_functions_count; i++)
		if ( !fscanf(conf, "%u ", (unsigned int *)&ann->cascade_activation_functions[i]) ) printf("fread error\n");
	...
	if ( !fscanf(conf, "cascade_activation_steepnesses=") ) printf("fread error\n");
	for(i = 0; i < ann->cascade_activation_steepnesses_count; i++)
		if ( !fscanf(conf, FANNSCANF" ", &ann->cascade_activation_steepnesses[i]) ) printf("fread error\n");
	...
	if ( !fscanf(conf, "layer_sizes=") ) printf("fread error\n");
...
#define SCALE_LOAD( what, where )											\
	if ( !fscanf( conf, #what "_" #where "=" ) ) printf("fread error\n");									\
	for(i = 0; i < ann->num_##where##put; i++)								\
	{																		\
...
#undef SCALE_LOAD
	...
	if ( !fscanf(conf, "neurons (num_inputs, activation_function, activation_steepness)=") ) printf("fread error\n");
	...
	if ( ! fscanf(conf, "connections (connected_to_neuron, weight)=") ) printf("fread error\n");
	for(i = 0; i < ann->total_connections; i++)
	{
