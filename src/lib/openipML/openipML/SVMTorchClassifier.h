#ifndef SVMTORCHCLASSIFIER_H
#define SVMTORCHCLASSIFIER_H

#include <openipML/Classifier.h>
#include <openipML/svmtorch/StandardSVM.h>
...
namespace openip
{
     ...
     */
    template<typename T>
    class SVMTorchClassifier: public virtual Classifier<T>
    {
    public:
         ...
         * default constructor
         */
        SVMTorchClassifier(int kernelType= RBF, int degree= 3, double gamma= 1.0, double C= 1.0, double coef0= 1.0, double cacheSize= 100, double eps= 0.01, bool norm= true, KernelFunction<double>* kernel= NULL);

        /**
         * copy constructor
         * @param s instance to copy
         */
        SVMTorchClassifier(const SVMTorchClassifier& s);
    ...
    template<typename T>
    SVMTorchClassifier<T>::SVMTorchClassifier(int kernelType, int degree, double gamma, double C, double coef0, double cacheSize, double eps, bool norm, KernelFunction<double>* kernel)
        : Classifier<T>()
    {
    ...
    template<typename T>
    SVMTorchClassifier<T>::SVMTorchClassifier(const SVMTorchClassifier& s)
        : Classifier<T>(s)
    {
    ...
    template<typename T>
    SVMTorchClassifier<T>::~SVMTorchClassifier()
    {
    ...
    template<typename T>
    int SVMTorchClassifier<T>::train(DataSet<T>& data, Vector<unsigned char>* mask)
    {
        //tprintf("training\n");
    ...
    template<typename T>
    void SVMTorchClassifier<T>::destroyProblem()
    {

    }

    template<typename T>
    int SVMTorchClassifier<T>::classify(Vector<T>& c, Vector<float>* probabilities)
    {
    ...
    template<typename T>
    int SVMTorchClassifier<T>::saveModelToFile(const char* filename)
    {
    ...
    template<typename T>
    int SVMTorchClassifier<T>::openModelFromFile(const char* filename)
    {
        /*std::ifstream file;
        file.open(filename);
    ...
    template<typename T>
    int SVMTorchClassifier<T>::saveModelToFile(ofstream& file)
    {
    ...
    template<typename T>
    int SVMTorchClassifier<T>::openModelFromFile(ifstream& file)
    {
